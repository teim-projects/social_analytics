...
# ------------------ Bright Color Theme (OPTION 2) ------------------
BRIGHT_LINE_COLORS = px.colors.qualitative.Vivid         # line, scatter
BRIGHT_BAR_COLORS = px.colors.qualitative.Bold           # bar charts
BRIGHT_CATEGORY_COLORS = px.colors.qualitative.Prism     # category bars
...
defaults = {
    ...
    "search_text": "",
    "selected_tags": [],
    "clear_filters_flag": False  # flag to trigger rerun
}
...
def clear_all_filters():
    for k, v in defaults.items():
        st.session_state[k] = v
    # Toggle flag to force rerun
    st.session_state["clear_filters_flag"] = not st.session_state["clear_filters_flag"]
...
if "Tags" in df.columns:
    ...
    top_tags = token_counts.head(200).index.tolist()

# Multiselect tied to session_state
...
if "Performance Quadrant (Views vs Like Rate)" in st.session_state.selected_graphs:
    ...
    fig = px.scatter(
        df_filtered, x="log_Views", y="Like_Rate", color="Perf_Quadrant",
        hover_data=["Title","Views","Likes"], 
        color_discrete_sequence=BRIGHT_CATEGORY_COLORS
    )
    fig.add_vline(x=views_med)
    fig.add_hline(y=like_med)
    plot_with_summary(fig, f"Median(log10 Views)={views_med:.2f}, Median(Like Rate)={like_med:.4f}")
...
if "Views Distribution (log10)" in st.session_state.selected_graphs:
    ...
    fig = px.histogram(
        df_filtered, x=np.log10(df_filtered["Views"]), nbins=40,
        labels={"x":"log10(Views)"}, 
        color_discrete_sequence=BRIGHT_BAR_COLORS
    )
    plot_with_summary(fig, f"Median views: {int(df_filtered['Views'].median())}")
...
if "Views vs Likes (log–log)" in st.session_state.selected_graphs:
    ...
    if mask.any():
        fig = px.scatter(
            df_filtered[mask],
            x=np.log10(df_filtered[mask]["Likes"]),
            y=np.log10(df_filtered[mask]["Views"]),
            hover_data=["Title","Views","Likes"],
            opacity=0.6,
            color_discrete_sequence=BRIGHT_LINE_COLORS
        )
...
if "Views vs Comments (log–log)" in st.session_state.selected_graphs:
    ...
    if mask2.any():
        fig = px.scatter(
            df_filtered[mask2],
            x=np.log10(df_filtered[mask2]["Comments_Count"]),
            y=np.log10(df_filtered[mask2]["Views"]),
            hover_data=["Title","Views","Comments_Count"],
            opacity=0.6,
            color_discrete_sequence=BRIGHT_LINE_COLORS
        )
...
if "Monthly Views (Growth Over Time)" in st.session_state.selected_graphs:
    ...
    if df_filtered["Published_At"].notna().any():
        ...
        fig = px.line(
            monthly, x="Published_At", y="Views",
            color_discrete_sequence=BRIGHT_LINE_COLORS
        )
...
if "Duration Distribution" in st.session_state.selected_graphs:
    ...
    fig = px.histogram(
        df_filtered, x="Duration_sec", nbins=40,
        color_discrete_sequence=BRIGHT_BAR_COLORS
    )
    plot_with_summary(fig, "- Duration distribution")
...
if "Duration vs Views (color = Like Rate)" in st.session_state.selected_graphs:
    ...
    fig = px.scatter(
        df_filtered, x="Duration_sec", y="Views", color="Like_Rate",
        color_continuous_scale=BRIGHT_CONTINUOUS,
        hover_data=["Title"]
    )
    plot_with_summary(fig, "- Do longer videos get more views?")

# 8. Duration Sweet Spot
if "Duration Sweet Spot (median views by bucket)" in st.session_state.selected_graphs:
    st.subheader("8️⃣ Duration Sweet Spot")
    bins = [0,5,10,15,30,60,np.inf]
    labels = ["0–5","5–10","10–15","15–30","30–60","60+"]
    df_filtered["Duration_Bucket"] = pd.cut(df_filtered["Duration_min"], bins=bins, labels=labels)
    duration_group = df_filtered.groupby("Duration_Bucket")["Views"].median().reset_index().dropna()
    fig = px.bar(
        duration_group, x="Duration_Bucket", y="Views",
        color_discrete_sequence=BRIGHT_BAR_COLORS
    )
    plot_with_summary(fig, "- Median views by duration bucket")

# 9. Upload Heatmap
if "Upload Time Heatmap (Avg Views by day/hour)" in st.session_state.selected_graphs:
    st.subheader("9️⃣ Upload Time Heatmap")
    ...
    fig = px.density_heatmap(
        heat, x="Hour", y="DayName", z="Views",
        color_continuous_scale=BRIGHT_CONTINUOUS
    )
    plot_with_summary(fig, "- Best upload time")

# 10. Average Views by Hour
if "Average Views by Upload Hour" in st.session_state.selected_graphs:
    ...
    fig = px.bar(
        hourly, x="Hour", y="Views",
        color_discrete_sequence=BRIGHT_BAR_COLORS
    )
    plot_with_summary(fig, "- Performance by upload hour")

# 11. Title Length vs Views
if "Title Length vs Views" in st.session_state.selected_graphs:
    st.subheader("1️⃣1️⃣ Title Length vs Views")
    fig = px.scatter(
        df_filtered, x="Title_Length", y="Views",
        hover_data=["Title"], trendline="ols",
        color_discrete_sequence=BRIGHT_LINE_COLORS
    )
    plot_with_summary(fig, "- Correlation between title length & views")

# 12. Engagement Funnel
if "Engagement Funnel (Like & Comment Rate)" in st.session_state.selected_graphs:
    st.subheader("1️⃣2️⃣ Engagement Funnel — Avg Rates")
    rates = pd.DataFrame({
        "Metric":["Like Rate","Comment Rate"],
        "Value":[df_filtered["Like_Rate"].mean(), df_filtered["Comment_Rate"].mean()]
    })
    fig = px.bar(
        rates, x="Metric", y="Value",
        color_discrete_sequence=BRIGHT_BAR_COLORS
    )
    plot_with_summary(fig, "- Average engagement")

# 13. Hidden Gems
if "Hidden Gems (High Like Rate, Low Views)" in st.session_state.selected_graphs:
    st.subheader("1️⃣3️⃣ Hidden Gems")
    views_25 = df_filtered["Views"].quantile(0.25)
    like_75 = df_filtered["Like_Rate"].quantile(0.75)
    hidden = df_filtered[(df_filtered["Views"] <= views_25) & (df_filtered["Like_Rate"] >= like_75)]
    if not hidden.empty:
        st.write(hidden[["Title","Views","Likes","Like_Rate"]].head(15))
        fig = px.scatter(
            hidden, x="Views", y="Like_Rate",
            color_discrete_sequence=BRIGHT_CATEGORY_COLORS
        )
        plot_with_summary(fig, "- High engagement, low reach")
    else:
...
if "Tag Count vs Views" in st.session_state.selected_graphs:
    ...
    fig = px.box(
        df_filtered, x="Tag_Count", y="Views",
        color_discrete_sequence=BRIGHT_CATEGORY_COLORS
    )
    plot_with_summary(fig, "- Effect of number of tags")
