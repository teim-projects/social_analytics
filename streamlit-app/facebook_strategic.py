...
# TITLE
# ============================
st.title("📊 Facebook Strategic Analytics Dashboard")

# ============================
# VIRAL DAYS
# ============================
threshold = filtered_df["Views"].mean() + 2 * filtered_df["Views"].std()
viral_days = filtered_df[filtered_df["Views"] > threshold]

col1, col2 = st.columns([2, 1])

with col1:
    fig = px.scatter(viral_days, x="Date", y="Views", size="Views", title="Viral Days")
    st.plotly_chart(fig, use_container_width=True)

with col2:
    st.markdown(f"""
    ### 📌 Insights
    - Viral threshold: **{threshold:.2f} views**
    - Total viral days: **{len(viral_days)}**
    - Highlights exceptional performance spikes.
    """)

# ============================
# FUNNEL
# ============================
funnel = filtered_df[["Views", "Visits", "Interactions", "Follows"]].sum()

col1, col2 = st.columns([2, 1])

with col1:
    fig = px.bar(x=funnel.index, y=funnel.values, title="Social Media Funnel")
    st.plotly_chart(fig, use_container_width=True)

with col2:
    st.markdown("""
    ### 📌 Insights
    - Shows user journey from Views → Follows.
    - Drop-offs indicate optimization areas.
...
# HIGH CTR DAYS
# ============================
high_ctr_threshold = filtered_df["Click_Through_Rate"].mean() + filtered_df["Click_Through_Rate"].std()
high_ctr_days = filtered_df[filtered_df["Click_Through_Rate"] > high_ctr_threshold]

col1, col2 = st.columns([2, 1])

with col1:
    fig = px.scatter(high_ctr_days, x="Date", y="Click_Through_Rate", size="Link_Clicks",
                     title="High CTR Days")
    st.plotly_chart(fig, use_container_width=True)
...
with col2:
    st.markdown(f"""
    ### 📌 Insights
    - Threshold: **{high_ctr_threshold:.4f}**
    - High-performing days: **{len(high_ctr_days)}**
    - Indicates strong CTA effectiveness.
    """)
